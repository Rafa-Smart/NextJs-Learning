...
export default function TemplateAbout({children}:{children:React.ReactNode}){
    ...
    // jadi kalo akses / saja maka hilang statenya


    // ITU SALAH YA sengaja engga di ahpus biar tau aja pernah mikir gitu

    // jadi kalo template itu di tipa halamannya itu akan hilang statenya


    return(
        <div>
            <button onClick={() => setCounter(counter +1)}>template {counter}</button>
            {children}
        </div>
    )
}
